...
const PROVIDERS = {
  openai: {
    async generate({ messages }) {
      ...
      /*
       * IMPORTANT:
       * Responses API accepts message content as a plain string.
       * Using { type: "input_text" } for assistant history is wrong;
       * that caused the "Supported values are output_text and refusal"
       * error seen in the previous build.
       *
       * Plain strings work for both user input and previous assistant
       * messages and preserve the conversation history cleanly.
       */
